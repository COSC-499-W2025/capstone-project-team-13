...
def check_contributor_status():
    """Check the current status of contributors for projects in database"""
    print("\n" + "="*60)
    print("Git Contributor Status Checker")
    print("="*60)
    ...
    projects_with_contributors = 0
    projects_without_contributors = 0
    total_contributors = 0
    git_repos = 0
    
    for project in projects:
        is_git = is_git_repository(project.file_path) if os.path.exists(project.file_path) else False
        if is_git:
            git_repos += 1
        ...
        if contributors:
            projects_with_contributors += 1
            total_contributors += len(contributors)
            print(f"Contributors: {len(contributors)} stored in database")
            print(f"\nContributor Details:")
            
            # Sort by commits
            sorted_contribs = sorted(contributors, key=lambda c: c.commit_count, reverse=True)
            for contrib in sorted_contribs:
                print(f"  • {contrib.name} <{contrib.contributor_identifier}>")
                print(f"    Commits: {contrib.commit_count:,} | "
                      f"+{contrib.lines_added:,}/-{contrib.lines_deleted:,} | "
                      f"{contrib.contribution_percent}%")
        else:
            projects_without_contributors += 1
            print(f"Contributors: None stored in database")
            if is_git:
                print(f"  ℹ️  This is a Git repository - contributors can be extracted")
    ...
    print(f"Git repositories: {git_repos}")
    print(f"Projects with contributors: {projects_with_contributors}")
    print(f"Projects without contributors: {projects_without_contributors}")
    print(f"Total contributors stored: {total_contributors}")
    print("="*60)
